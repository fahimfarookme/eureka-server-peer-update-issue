...
function update_in_config_repo {
	echo "eureka.client.service-url.defaultZone=http://host-${host}:888${host}/eureka" > application.properties
...
java $DEBUG -Dconfig.server.uri=http://localhost:$CONFIG_SERVER_PORT -Dport=14001 -jar eureka-server/target/microservice-0.0.1-SNAPSHOT.jar &
